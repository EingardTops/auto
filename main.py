...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    CUSTOMER main Favorite car: {Favorite car};
    TASK: Write a product description that is tailored into this customer's Age group and Favorite car. Use age group specific slang.;
    ...
    BENEFITS: describe in 3 sentences why this product is perfect considering customers age group and Favorite car;
    USE CASE: write a story in 5 sentences, of an example weekend activity taking into account Favorite car {Favorite car} and age {agegroup}, write a story in first person, example "I started my Saturday morning with ...";
"""

prompt = PromptTemplate(
    input_variables=["agegroup", "Favorite car", "content"],
    template=template,
)
...
def get_hobby():
    input_text = st.text_input(label="Customers Favorite car", key="hobby_input")
    return input_text
...
if content_input:
    ...
    prompt_with_content = prompt.format(agegroup=option_agegroup, Favorite car=hobby_input, content=content_input)
